...
import com.quseit.util.ImageRequest;
import com.quseit.util.ImageRequest.ImageRequestCallback;
...
public class AsyncImageView extends ImageView implements ImageRequestCallback {
    ...
    public void setImageBitmap(Bitmap bitmap) {
		...
		byte[] bitmapBt = ImageUtil.Bitmap2Bytes(bitmap);
		...
		//bitmap = BitmapFactory.decodeByteArray(bitmapBt,0,bitmapBt.length, bitmapOptions);
        try {
        	...
        	Log.d("AsyncImageView", "setImageBitmap ooe");
			e.printStackTrace();
            ...
            switch (mImageSource) {
                case IMAGE_SOURCE_BITMAP:
                	if (NAction.getExtP(getContext(), "conf_image_round").equals("1")) {
                		...
                		setImageBitmap(ImageUtil.toRoundCorner(mDefaultBitmap));
                	} else {
                ...
                case IMAGE_SOURCE_DRAWABLE:
                	//add roundcorner
                	BitmapDrawable bd = (BitmapDrawable) mDefaultDrawable;

                	if (NAction.getExtP(getContext(), "conf_image_round").equals("1")) {
                    	setImageBitmap(ImageUtil.toRoundCorner(bd.getBitmap()));

                	} else {
                		setImageBitmap(bd.getBitmap());
                	}
                	//Bitmap bm = ImageUtil.toRoundCorner(bd.getBitmap());
